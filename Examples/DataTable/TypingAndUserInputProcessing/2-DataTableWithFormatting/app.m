...
customColumns{2} = struct('id','max','name','Max Temperature (˚F)','type','numeric',...
    'format', py.dash_table.Format.Format(pyargs(...
        'precision',py.int(0),...
        'scheme',scheme.fixed,...
        'symbol',symbol.yes,...
        'symbol_suffix','˚F')));
customColumns{3} = struct('id','max_date','name','Max Temperature (Date)','type','datetime');
customColumns{4} = struct('id','min','name','Min Temperature (˚F)','type','numeric',...
    'format', py.dash_table.Format.Format(pyargs(...
    'nully','N/A',...
        'precision',py.int(0),...
        'scheme',scheme.fixed,...
...
customColumns{5} = struct('id','min_date','name','Min Temperature (Date)','type','datetime',...
